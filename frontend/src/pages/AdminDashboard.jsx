...
import { fetchDashboardStats, fetchMonthlyRevenue } from '../redux/slices/adminSlice.js';
import { AreaChart, Area, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer } from 'recharts';
import { TrendingUp, Users, Package, ShoppingBag, AlertTriangle, ArrowUpRight, ChevronRight, Activity } from 'lucide-react';

const AdminDashboard = () => {
  const dispatch  = useDispatch();
  const navigate  = useNavigate(); // ✅ ALL hooks before any early return
  ...
  // ✅ guard against undefined monthlyRevenue — slice initialises it as []
      ...
      <div className="max-w-7xl mx-auto space-y-10">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {topStats.map((s, i) => (
            <div key={i} className="group p-8 rounded-[2.5rem] bg-[#0d0d0c] border border-white/5 hover:border-[#958E62]/30 transition-all duration-700 shadow-2xl relative overflow-hidden">
              <div className="absolute top-0 left-0 w-full h-px bg-gradient-to-r from-transparent via-[#958E62]/10 to-transparent" />
              <div className="flex justify-between items-start mb-6">
                <div className={`w-12 h-12 rounded-2xl bg-white/[0.02] border border-white/5 flex items-center justify-center ${s.color}`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <AreaChart data={revenueChartData}>
                    ...
                    <XAxis dataKey="name" axisLine={false} tickLine={false}
                           tick={{ fill: '#ffffff33', fontSize: 10, fontFamily: 'JetBrains Mono' }} dy={15} />
                    ...
                    />
                    <Area type="monotone" dataKey="revenue" stroke="#958E62" strokeWidth={3}
                          fillOpacity={1} fill="url(#colorRev)" animationDuration={2000} />
                  </AreaChart>
                </ResponsiveContainer>
              </div>
            ) : (
              <div className="h-72 flex items-center justify-center">
                <p className="font-['JetBrains_Mono'] text-[10px] uppercase tracking-widest text-white/15">No revenue data yet</p>
              </div>
            )}
          </div>
          ...
          <div className="lg:col-span-4">
            <div className="p-8 rounded-[3rem] bg-[#0d0d0c] border border-white/5 h-full flex flex-col">
              ...
              <div className="space-y-4 flex-1">
                {stats?.lowStockWatches?.length > 0 ? (
                  stats.lowStockWatches.map((w) => (
                    <div key={w._id} className="flex items-center gap-4 p-4 rounded-2xl bg-white/[0.02] border border-white/5 hover:border-white/10 transition-all group">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="font-['JetBrains_Mono'] text-[9px] uppercase text-amber-400/70 mt-0.5">{w.quantity} units remaining</p>
                      </div>
                    </div>
                  ))
                ) : (
                  <div className="flex-1 flex items-center justify-center">
                    <p className="font-['JetBrains_Mono'] text-[10px] text-white/20 uppercase tracking-widest">Inventory healthy</p>
                  </div>
                )}
              </div>
              <Link to="/admin/products"
                    className="flex items-center justify-between w-full mt-8 p-4 rounded-2xl border border-white/5
                               font-['JetBrains_Mono'] text-[9px] uppercase tracking-[0.2em] text-white/30
                               hover:text-[#958E62] hover:border-[#958E62]/20 transition-all group">
                Manage Registry
        ...
        <div className="p-8 md:p-10 rounded-[3rem] bg-[#0d0d0c] border border-white/5 overflow-hidden">
          ...
          {/* ✅ guard against empty recentOrders */}
          {stats?.recentOrders?.length > 0 ? (
                ...
                <tbody className="divide-y divide-white/[0.03]">
                  {stats.recentOrders.map((order) => (
                    <tr key={order._id}
                      ...
                      <td className="py-6">
                        <span className="px-3 py-1 rounded-full border border-white/10 font-['JetBrains_Mono'] text-[8px] uppercase text-white/40 tracking-widest">
                          {order.orderStatus}
                        </span>
                      </td>
            ...
            <div className="py-16 flex items-center justify-center">
              <p className="font-['JetBrains_Mono'] text-[10px] uppercase tracking-widest text-white/15">No orders yet</p>
            </div>
          )}
        </div>
